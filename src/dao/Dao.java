...
//            rs=st.executeQuery(sql);              //执行sql语句
    ...
    //检索，查询是否注册
    public int jiansuo(String name) {
        Connection conn = Shujuku.connect();
        Statement st = null;
        ResultSet rs = null;
        boolean c = false;
    ...
    public boolean jiansuo1(String name, int score) {
        Connection conn = Shujuku.connect();
        Statement st = null;
        ResultSet rs = null;
        boolean c = false;
    ...
    public boolean Shanchu(String name) {
        ...
        int c = 0;
        try {
            String sql = "delete from inf where name ='" + name + "'";
            st = conn.createStatement();        //建立一个Connection对象
            c = st.executeUpdate(sql);              //执行sql语句
            if (c > 0) {
                return true;
            }
        } catch (Exception e) {
